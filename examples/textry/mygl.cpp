...
// 	
// // 	
// // 	mm::Image* img = new mm::SoilImage("square2.jpg");
// 	mm::Texture tex("square2.jpg",GL_CLAMP_TO_EDGE,GL_LINEAR);
// 	
// 	glm::mat4 p = glm::perspective(60.0f,1000*1.0f/600,0.1f,100.0f);
// 	glm::mat4 v = glm::translate(glm::mat4(1.0f), glm::vec3(-5.0f, -5.0f, -50.0f)); // Create our view matrix which will translate us back 5 units  
...
int main()
{
	mm::Framework f;
	glClearColor(1,1,1,1);
	mm::OldMesh m("squaredata","tvert.glsl","tfrag.glsl");
	mm::Texture t("square2.jpeg",GL_CLAMP_TO_EDGE,GL_LINEAR);
	while(f.update()!=-1)
	{
		glClear(GL_COLOR_BUFFER_BIT);
		m.draw();
